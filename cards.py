...
UA = {"User-Agent": "Mozilla/5.0 (compatible; abod-leaks-bot/1.4)"}
VBUCK = "https://fortnite-api.com/images/vbuck.png"
COLS, TILE_W, TILE_H, GAP, PAD = 6, 250, 340, 18, 48
SEC_HEAD, SEC_GAP = 74, 30                        # a section title row, and the space between sections
WIDTH = PAD * 2 + COLS * TILE_W + (COLS - 1) * GAP
FIXED = 40 + 190 + 24 + 6 + 28 + 70 + 20          # card height minus the sections (see the CSS below)
...
h2{height:64px;margin-bottom:10px;display:flex;align-items:center;gap:16px;
  font-family:Changa;font-weight:800;font-size:44px;line-height:1}
h2 b{font-family:Tajawal;font-weight:800;font-size:26px;line-height:1;padding:7px 16px 9px;border-radius:16px;
  color:#0b0620;background:linear-gradient(90deg,#8b5cf6,#22d3ee)}
h2:after{content:'';flex:1;height:3px;border-radius:2px;
  background:linear-gradient(270deg,rgba(139,92,246,.85),rgba(34,211,238,0))}
.grid{position:relative;display:grid;grid-template-columns:repeat({{COLS}},{{TW}}px);gap:{{GAP}}px}
.tile{position:relative;width:{{TW}}px;height:{{TH}}px;border-radius:20px;overflow:hidden;
  background:linear-gradient(180deg,var(--a),var(--b));box-shadow:0 10px 24px rgba(0,0,0,.4)}
.art{position:absolute;left:0;right:0;top:0;height:250px;display:flex;align-items:flex-end;justify-content:center}
.art:before{content:'';position:absolute;left:40px;right:40px;top:30px;bottom:6px;border-radius:50%;
  background:rgba(255,255,255,.34);filter:blur(30px)}
.art img{position:relative;max-width:244px;max-height:244px}
.badge{position:absolute;top:10px;right:10px;padding:3px 12px 5px;border-radius:14px;background:rgba(9,5,26,.72);
  font-weight:800;font-size:17px}
.bar{position:absolute;left:0;right:0;bottom:0;height:90px;padding:0 12px;background:rgba(9,5,26,.93);
  border-top:3px solid var(--edge);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px}
.name{max-width:100%;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.price{display:flex;align-items:center;gap:7px;direction:ltr;font-weight:800;font-size:26px}
.price img{width:26px;height:26px}
.price s{font-size:19px;color:#a5a5be;text-decoration-color:#f45260;text-decoration-thickness:3px}
.line{max-width:100%;font-weight:700;font-size:20px;color:#cfcfe8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
footer{position:relative;height:70px;display:flex;align-items:flex-end;justify-content:center;
...
def name_px(name):
    n = len(name)
    return 25 if n <= 12 else 22 if n <= 16 else 20 if n <= 21 else 17
...
def draw_card(title, subtitle, sections, footer, avatar_url=None):
    """Returns JPEG bytes: a branded header, then each (section title, tiles) as a titled grid,
    six tiles per row, right-to-left. A plain list of tiles is drawn as one untitled section."""
    if sections and isinstance(sections[0], dict):
        sections = [(None, sections)]
    sections = [(name, tiles) for name, tiles in sections if tiles]
    body = 0
    for i, (name, tiles) in enumerate(sections):
        rows = (len(tiles) + COLS - 1) // COLS
        body += (SEC_HEAD if name else 0) + rows * TILE_H + (rows - 1) * GAP
        body += SEC_GAP if i < len(sections) - 1 else 0
    height = FIXED + max(body, TILE_H)
    tiles = [t for _, ts in sections for t in ts]
    with tempfile.TemporaryDirectory() as tmp:
        ...
        esc = html.escape
        blocks, k = [], 0
        for i, (name, ts) in enumerate(sections):
            head = f"<h2><span>{esc(name)}</span><b>{len(ts)}</b></h2>" if name else ""
            grid = "".join(tile_html(t, arts[k + j], vbuck) for j, t in enumerate(ts))
            k += len(ts)
            gap = 0 if i == len(sections) - 1 else SEC_GAP
            blocks.append(f'<section style="margin-bottom:{gap}px">{head}<div class="grid">{grid}</div></section>')
        ...
        page = (f'<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><style>{css}</style></head>'
                ...
                f'<div class="sub">{sub}</div></div>{brand}</header><div class="rule"></div>'
                + "".join(blocks) + f"<footer>{esc(footer)}</footer></div></body></html>")
